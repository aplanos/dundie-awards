package com.ninjaone.dundieawards.organization.infrastructure.config;

import com.ninjaone.dundieawards.organization.infrastructure.config.tenants.FlywayMigrationInitializer;
import com.ninjaone.dundieawards.organization.infrastructure.config.tenants.MultiTenantSchemaConnectionProvider;
import com.ninjaone.dundieawards.organization.infrastructure.config.tenants.SchemaTenantIdentifierResolver;
...
@EnableJpaRepositories(basePackages = "com.ninjaone.dundieawards.organization.infrastructure.repository",
        entityManagerFactoryRef = "organizationEntityManagerFactory",
        transactionManagerRef= "organizationTransactionManager"
)
@Qualifier("organizationDatasourceConfig")
public class OrganizationDataSourceConfig {

    @Bean
    @ConfigurationProperties("spring.datasource")
    @Qualifier("organizationDataSourceProperties")
    public DataSourceProperties organizationDataSourceProperties() {
    ...
    public DataSource organizationDataSource() {
        return organizationDataSourceProperties().initializeDataSourceBuilder()
                .type(HikariDataSource.class).build();
    }

    @Bean(initMethod = "migrate")
    public FlywayMigrationInitializer flywayMigrationInitializer() {
        return new FlywayMigrationInitializer(organizationDataSource());
    }
    ...
    public MultiTenantConnectionProvider<String> multiTenantConnectionProvider() {
        return new MultiTenantSchemaConnectionProvider(organizationDataSource());
    }
